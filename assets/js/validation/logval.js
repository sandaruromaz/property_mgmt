...
/* 
	...
	*/
    ...
    var x=$('#emaillogin').val();
    var atpos=x.indexOf("@");
    var dotpos=x.lastIndexOf(".");
    if($('#emaillogin').val() == ''){//check errormessage empty or not. if not empty there are errors.
        error_display($('#emaillogin'),'Email must be entered');
        errormessage += 1;
    }else if(atpos<1 || dotpos<atpos+2 || dotpos+2>=x.length)
    {
      error_display($('#emaillogin'),'Email Address is not valid.');
	  errormessage += 1;
...
/*	
	...
	errormessage += 1;
	 ...
	 if($('#passlogin').val() == ''){//check errormessage empty or not. if not empty there are errors.
        error_display($('#passlogin'),'Your password must be entered');
       errormessage += 1; 
    }else if ($('#passlogin').val().length<7)
		   {
	    error_display($('#passlogin'),'Your password should be more than seven characters. ');
		errormessage += 1;
		}		
