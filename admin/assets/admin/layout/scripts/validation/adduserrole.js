...
function adduserrolevalidation(){	
    ...
    //User Role Name
	if($('#userrolename').val() == ''){//check errormessage empty or not. if not empty there are errors.
	error_display($('#userrolename'),'User Role Name must be entered');
	errormessage += 1; 
	} 	else if(!$('#userrolename').val().match(/^[a-zA-Z ]+$/)){
	error_display($('#userrolename'), 'User Role Name must be alphabet characters only');
	errormessage += 1;
	}
