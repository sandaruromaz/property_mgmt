...
function editcatevalidation(){	
 ...
 var errormessage="";
    ...
    if($('#catname1').val() == ''){//check errormessage empty or not. if not empty there are errors.
        error_display($('#catname1'),'Category Name must be entered');
       errormessage += 1; 
    } 	else if(!$('#catname1').val().match(/^[a-zA-Z-]+$/) ){
	 error_display($('#catname1'), 'Category Name must be alphabet characters only');
    errormessage += 1;
	}
